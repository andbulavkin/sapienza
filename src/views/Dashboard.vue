    ...
    setGraph() {
      ...
      if (this.selectedComparment != 0 && this.selectedBatch != 0) {
        ...
        if (
          ...
          this.$http
            .get(this.$apiBaseUrl + "dashboard/graph/" + bathcId+ "/"+ this.type, {
              headers: {
                Authorization: this.accessToken(),
              },
            })
            .then((response) => {
              if (response.status == 200) {
                ...
                let length = response.data.data.eC[1].lable.length;
                ...
                for (
                  let index = 0;
                  index < response.data.data.eC.length;
                  index++
                ) {
                  const element = response.data.data.eC[index];
                ...
                for (
                  let index = 0;
                  index < response.data.data.pH.length;
                  index++
                ) {
                  const element = response.data.data.pH[index];
