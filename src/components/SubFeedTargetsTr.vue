...
export default {
    ...
    methods: {
        getData() {
            this.$http
                .get(this.$apiBaseUrl + "feed-sub/" + this.ParentId, {
                    headers: {
        ...
        store(index) {
            this.$http
                .post(
                    this.$apiBaseUrl + "feed-sub",
                    {
                        feedID: this.items[index].feedID,
        ...
        update(index) {
            this.$http
                .put(
                    this.$apiBaseUrl + "feed-sub/" + this.items[index].feedSubId,
                    {
                        fromDay: this.items[index].fromDay,
                        toDay: this.items[index].toDay,
                        ecMinMax: this.items[index].ecMinMax,
        ...
        openDelete(id) {
            this.deleteId = this.items[id].feedSubId;
            $("#DeleteSubField").modal("show");
        },
        deleteSubData() {
            if (this.deleteId != "") {
                this.$http
                    .delete(this.$apiBaseUrl + "feed-sub/" + this.deleteId, {
                        headers: {
                            Authorization: this.accessToken(),
                        },
                    })
        ...
        removeEmpty() {
            if (
                ...
                this.items[this.items.length - 1].feedSubId == ""
            ) {
                this.items.pop();
            }
        },
